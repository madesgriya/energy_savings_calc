...
class get_data():
    ...
    def json_input(self):
        ...
        json_array = {
            ...
            'buildingSize': response['buildingSize'],
            'sysType': response['sysType'],
            'curSP': response['curSP'],
            'humidity': response['humidity'],
            'desiredSP': response['desiredSP']
        }
        return json_array
...
class prelim_data:
    def __init__(self):
        ...
        #self.area = float(input("Enter the area of the building in square meters: "))
        self.facility = get_data().json_input()['facility']
        self.equip = get_data().json_input()['sysType']
        self.area = get_data().json_input()['buildingSize']
        self.cursp = get_data().json_input()['curSP']
        self.humidity = get_data().json_input()['humidity']
        self.desiredsp = get_data().json_input()['desiredSP']
    ...
    def enthalpy_diff(self):
        ...
        elec_bill = 0.12 #S$/kWh  #apply different tarrif for different regions
    ...
    # add suggestions for the user based on the energy saving, and trivial number for each categories of building 
    # add general recomendation from bodies e.g. BCA 
    # ask email after result for deeper analysis 

def lambda_handler(): #params needed for AWS Lambda
